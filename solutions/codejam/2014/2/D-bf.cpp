...
long long rec(int v, int depth) {
	...
	if (end[v]) {
		...
		for (int not_occupied = 0; not_occupied <= servers; not_occupied++) {
			for (int free_servers = 0; free_servers <= servers; free_servers++) {
				int occupy_free = free_servers-not_occupied;
				long long th = (pd[depth][free_servers] * arrange[servers-free_servers][(not_occupied+occupies)-free_servers]) % mod;
				th = (th * choose[occupies][(not_occupied+occupies)-free_servers]) % mod;
				t[depth][not_occupied] = (t[depth][not_occupied] + th) % mod;
			}
		}
